...
from tools.plugins import get_input_wh


class DarkNetParser(object):
    """Definition of a parser for DarkNet-based YOLO model."""

    def __init__(self, supported_layers, cfg_file_path):
    ...
    def verify_classes(self, num_classes):
    ...
    def _next_layer(self, remainder):
        ...
        elif layer_type == 'region': layer_param_lines = []
...
class ConvParams(object):
    ...
    def __init__(self, node_name, batch_normalize, conv_weight_dims, groups):
        ...
        groups -- group value for grouped convolutional layer
        """
...
class UpsampleParams(object):
    ...
    def generate_param_name(self, param_category):
        """Generates the scale parameter name for the Upsample node."""
        assert param_category in ['roi','scale']
        if param_category == 'roi':
            param_name = self.node_name + '_' + 'roi'
        elif param_category == 'scale':
...
class WeightLoader(object):
    ...
    def __init__(self, weights_file_path, yolo_version):
        ...
        #self.total_bytes = len(self.weights_file.read())
    ...
    def load_upsample_scales(self, upsample_params):
        ...
        initializer = list()
        inputs = list()
        roi_name = upsample_params.generate_param_name('roi')
        scale_name = upsample_params.generate_param_name('scale')
        shape = upsample_params.value.shape
        data = upsample_params.value
        roi_init = helper.make_tensor(
            roi_name, TensorProto.FLOAT, (0,), [])
        roi_input = helper.make_tensor_value_info(
            roi_name, TensorProto.FLOAT, (0,))
        
        scale_init = helper.make_tensor(
            scale_name, TensorProto.FLOAT, shape, data)
    ...
    def _open_weights_file(self, weights_file_path):
        ...
        if int(self.yolo_version.split("x")[0]) > 2:
            print("version over 2 has length_header = 5")
            length_header = 5
        else:
            print("version under or equal 2 has length_header = 4")
            length_header = 4
        _buffer = weights_file.read(length_header * 4)
        #self.accum_bytes += len(_buffer)
        #print("cur_size {}".format(len(_buffer)))
        #print("accum_size {}".format(self.accum_bytes))
...
class GraphBuilderONNX(object):
    ...
    def __init__(self, model_name, output_tensors, batch):
        ...
        self.yolo_version = self.model_name.split("-")[0].split("v")[-1] 
        print("yolo version is {}".format(self.yolo_version))
    ...
    def build_onnx_graph(
        ...
        weight_loader = WeightLoader(weights_file_path, self.yolo_version)
        initializer = list()
        ...
        #print(" >>>>>>    Start to build the {} model to ONNX".format(self.model_name))
        model_def = helper.make_model(self.graph_def,
                                      producer_name='NVIDIA TensorRT')
        return model_def
    ...
    def _make_onnx_node(self, layer_name, layer_dict):
        ...
        if self.input_tensor is None:
            ...
            node_creators['region'] = self._make_region_node
            node_creators['reorg'] = self._make_reorg_node
            ...
            #print("Make_onnx_node >>>>>    major_node_specs.name: ", major_node_specs.name)
    ...
    def _make_conv_node(self, layer_name, layer_dict):
        ...
        if 'size' in layer_dict.keys():
            kernel_size = layer_dict['size']
        else:
            raise ValueError('It is necessary configuring [size] for Convolutional')

        stride = layer_dict['stride'] if 'stride' in layer_dict.keys() else 1 

        if 'filters' in layer_dict.keys():
            filters = layer_dict['filters']
        else:
            raise ValueError('It is necessary configuring [filters] for Convolutional')
    ...
    def _make_upsample_node(self, layer_name, layer_dict):
        ...
        scales_name = upsample_params.generate_param_name('scale')
    ...
    def _make_yolo_node(self, layer_name, layer_dict):
        previous_node_specs = self._get_previous_node_specs()
        print(". yolo inputs :", previous_node_specs.name)
    ...
    def _make_region_node(self, layer_name, layer_dict):
        previous_node_specs = self._get_previous_node_specs()
        print(". region inputs :", previous_node_specs.name)
        """Create an ONNX Region node.
    ...
    def _make_reorg_node(self, layer_name, layer_dict):
        previous_node_specs = self._get_previous_node_specs()
        print(". reorg layer inputs :", previous_node_specs.name)
        """Create an ONNX Reorg node.

        These are dummy nodes which would be removed in the end.
        """
        blocksize = None
        if 'stride' in layer_dict.keys():
            blocksize = layer_dict['stride']
        else:
            blocksize = 1

        if 'reverse' in layer_dict.keys():
            print("[Warning] reverse feature is unavailable.")
            reverse = layer_dict['reverse']
            if reverse != 0 :
                raise TypeError('[Error] Reverse = 0 is the conly configurable')
        else:
            reverse = 0
...
def main():
    ...
    parser.add_argument(
        ...
        help=('Put YOLO model url.'
              'Samples are like [models/yolov[3|4][-nano|-lite|-tiny|-spp|-csp]-[288|416|608].weights'))
    parser.add_argument(
    ...
    model_name = args.model_file.split('/')[-1].split('.weights')[0]
    onnx_path = 'models/onnx/'

    if args.num_category <= 0:
        raise SystemExit('ERROR: bad num_category (%d)!' % args.num_category)
    if not os.path.isfile(args.model_file):
        raise SystemExit('ERROR: file (%s) not found!' % weights_file_path)
    config_file = '%s.cfg' % args.model_file.split(".weights")[0]
    if not os.path.isfile(config_file):
        raise SystemExit('ERROR: file (%s) not found!' % config_file)
    output_file = onnx_path + '%s.onnx' % model_name
    print("[INFO] output will be saved to ", output_file)
    ...
    supported_layers = ['net', 'convolutional', 'maxpool',
                        'shortcut', 'route', 'upsample', 'yolo', 'region', 'reorg']
    ...
    parser = DarkNetParser(supported_layers, config_file)

    if not parser.verify_classes(args.num_category):
        raise SystemExit('ERROR: bad num_category (%d)' % args.num_category)
    layer_configs = parser.parse_cfg_file()
    ...
    w, h = get_input_wh(model_name)

    # In above layer_config, there are three outputs that we need to
    # know the output shape of (in CHW format).
    output_tensor_dims = OrderedDict()
    if 'yolov2' in model_name:
        # layer_ch = num * (num_category + coords + 1)
        c = (args.num_category + 5) * 5 
        if 'tiny' in model_name:
            output_tensor_dims['015_convolutional'] = [c, h // 32, w // 32]
        else:
            output_tensor_dims['031_convolutional'] = [c, h // 32, w // 32]

    elif 'yolov3' in model_name:
        # layer_ch = len(mask) * (num_category + len([x,y,w,h] + 1)
        c = (args.num_category + 5) * 3
        if 'tiny' in model_name:
            output_tensor_dims['016_convolutional'] = [c, h // 32, w // 32]
            output_tensor_dims['023_convolutional'] = [c, h // 16, w // 16]
        elif 'spp' in model_name:
            ...
            print("nano have to know output_tensor_dims")
            output_tensor_dims['069_convolutional'] = [c, h // 32 , w // 32]
            output_tensor_dims['079_convolutional'] = [c, h // 16 , w // 16]
        elif 'lite' in model_name:
            print("lite have to know output_tensor_dims")
            output_tensor_dims['070_convolutional'] = [c, h // 32, w // 32]
            output_tensor_dims['080_convolutional'] = [c, h // 16, w // 16]
        else:
    ...
    builder = GraphBuilderONNX(model_name, output_tensor_dims, args.batch_size)
    ...
    yolo_model_def = builder.build_onnx_graph(
        ...
        weights_file_path=args.model_file,
        verbose=True)
    ...
    if args.batch_size < 0:
        yolo_model_def.graph.input[0].type.tensor_type.shape.dim[0].dim_param = '?'
    ...
    onnx.save(yolo_model_def, output_file)
